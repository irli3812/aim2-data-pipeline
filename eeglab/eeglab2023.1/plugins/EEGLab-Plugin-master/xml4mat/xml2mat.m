...
while do_i<=length(todo_list);
    ...
    if ~isfield(w,'class')
        w.class='char';
        % some starstim filenames contain special characters that produce
        % bugs in this part of the code. For example, if the filename
        % contains "20190619121732_SAMUELE_F3-F4-SHAM_tACS_6'_F3-P4" it
        % crashes. A potential hack below
        % tag_contents{i,4} = regexprep(tag_contents{i,4},"'",'p');
